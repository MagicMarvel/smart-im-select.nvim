...
local C = {
    ...
    -- alternative input method in normal mode.
    alternative_method_selected = '2052',
...
local function switch_to_alternative_im()
    local current = get_current_select(C.default_command)
    vim.api.nvim_set_var("im_select_saved_state", current)

    if current ~= C.alternative_method_selected then
        change_im_select(C.default_command, C.alternative_method_selected)
    end
end
